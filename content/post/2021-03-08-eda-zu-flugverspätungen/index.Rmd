...
```{r}
flights %>% 
  summarise(is_below_75_quantile = quantile(dep_delay, 
                                  prob = .75, 
                                  na.rm = TRUE))
```
...
flights2 <-
  ...
  mutate(is_extreme = case_when(
    dep_delay > 11 + 1.5 * 16 ~ TRUE,
    TRUE ~ FALSE  # in den anderen Fällen, dann kein Extremwert
  ))
```
...
Hier fassen wir mit `fct_lump` alle Stufen von `carrier` zu einer "Lumpensammler-Kategorie" zusammen, die weniger als 8 Mal vorkommen.
...
flights2 %>% 
  filter(!is_extreme) %>% 
  ggplot() +
  aes(x = fct_reorder(carrier, dep_delay, 
                      na.rm = TRUE), 
      y = dep_delay) %>% 
  geom_boxplot()
```
...
flights2 %>% 
  ...
  fastDummies::dummy_cols() %>%   # Dummysierung
  select(-carrier) %>% 
...
Keine Korrelation war größer als 0.1.
