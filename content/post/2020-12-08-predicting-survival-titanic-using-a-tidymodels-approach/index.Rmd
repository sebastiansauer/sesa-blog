...
titanic_recipe <- 
  ...
  update_role(Ticket, Cabin, Name, PassengerId, Embarked,  
              new_role = "ID") %>%   # Use "ID" as ID, not as predictor
  step_knnimpute(all_predictors(), neighbors = 3) %>%   # impute missing values
  #step_rm() %>%   # remove these vars
  step_dummy(all_predictors(), -all_outcomes()) %>%   # make dummy variables
  step_nzv(all_predictors()) %>%   # exclude near zero variance predictors
  step_corr(all_predictors()) %>%  # exclude highly correlated vars
  step_center(all_predictors(), -all_outcomes()) %>%  # center
  step_scale(all_predictors(), -all_outcomes()) # set sd=1
...
```{r}
last_rf_mod <- 
  rand_forest(mtry = 2, min_n = 5, trees = 1000) %>% 
