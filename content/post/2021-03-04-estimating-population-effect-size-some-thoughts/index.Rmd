...
In other words, given this model, we assume that the probability that the treatment is effective equals to $p_1 = 1-p_0$, that is
...
As can be seen, lager sample sizes, do precisely show the true parameter ($mu=0.5$).
...
Even the difference between $n=128$ and $n=128$ ($2^7, 2^8$) appears negligible, from a practical point if view, given the large uncertainties associated with research study results in practice. Let's remember that we should try to be more precise as it is possible (don't estimate the age of the earth in seconds, rather in millions of years).
