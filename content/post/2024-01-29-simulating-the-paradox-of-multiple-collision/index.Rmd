...
As can be seen, the probability of finding a matching time slot is quick small, given the parameter values above.
Not impossible, but too low to rely on this approach in my opinion.



# Make a matching more probable

Let's provide less options too chose from in order to find a time slot for the meeting to your colleagues:

```{r}
o <- 12  # time slot options too chose from
```



```{r draw-many-simulated-samples2}
smples <- 
  slots_chosen_collision |> 
  mutate(id = 1)
...
# Make a matching highly probable
...
```{r draw-many-simulated-samples3}
smples <- 
  slots_chosen_collision |> 
  mutate(id = 1)
...
Now we can see that a matching has become highly probable.
