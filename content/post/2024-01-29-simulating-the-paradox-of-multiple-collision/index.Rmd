...
sample_with_dependency <- function(p_set, o, dep) {
  ...
  slots_chosen_l[[1]] <- sample(c(p_set, u_set), 
                                size = p, 
                                prob = c(rep(Pr_Pi_new, p), rep(Pr_Ui_new, u)))
  
  return(slots_chosen_l[[1]])
}
```

Check:
```{r}
#debug(sample_with_dependency)
sample_with_dependency(p_set = c(1,2,3), o = o, dep = .6)
```
...
slots_chosen_collision <- count_common_slots(n_colleagues = n_colleagues, 
                                             o = o, 
                                             p = p,
                                             dep = dep)
...
```{r}
plot_matches(smples_tbl, n_colleagues, p = p, o = o, dep = dep)
```
...
```{r smpl_distrib1}
tic()
smpl_distrib <- mosaic::do(5) * matching_prob(n_colleagues = 3,
              o = 10,
              p = 2,
              dep = 0)
toc()
...
For simplicaty, let's only allow the values 0, .3, .6., .9 for dependency,
...
```{r}
sample_with_dependency(p_set = c(3, 1, 9), o = 10, d = .6)
```
...
```{r}
slots_chosen <- count_common_slots_with_dependency(3, 10, 3, .6)
slots_chosen
```

```{r}
attributes(slots_chosen)
```
...
```{r def-grid2}
d2 <- expand_grid(n_colleagues = c(3,5),
                 o = c(5, 10, 15),
                 p = c(3),
                 dep = c(0, .3, .9))
```
